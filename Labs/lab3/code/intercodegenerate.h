...
void translate_Exp(node* Exp, tablenode* sym_table, Operand place)
{
	if(Exp->numchild == 3)
	{
        ...
        strcmp(Exp->children[1]->name, "MINUS") == 0 ||
        strcmp(Exp->children[1]->name, "STAR") == 0 ||
        strcmp(Exp->children[1]->name, "DIV1") == 0){
			Operand t1 = new_temp();
			Operand t2 = new_temp();
			...
			if(strcmp(Exp->children[1]->name, "MINUS") == 0)op = "SUB1";
			if(strcmp(Exp->children[1]->name, "STAR") == 0)	op = "MUL1";
			if(strcmp(Exp->children[1]->name, "DIV1") == 0)	op = "DIV1";
...
void gen_LABEL_IR(InterCodes tmp, FILE* fp){
...
void gen_FUNCTION(InterCodes tmp, FILE* fp){
...
void gen_ASSIGN(InterCodes tmp, FILE* fp){
	char *l_name = tmp->code->u.assign.left->u.name;
	char *r_name = tmp->code->u.assign.right->u.name;
	int l_value = tmp->code->u.assign.left->u.value;
	int r_value = tmp->code->u.assign.right->u.value;

	if (tmp->code->u.assign.left->kind == VARIABLE) {
		if (tmp->code->u.assign.right->kind == VARIABLE) {
			fprintf(fp, "%s := %s\n", l_name, r_name);
		} else if (tmp->code->u.assign.right->kind == CONSTANT) {
			fprintf(fp, "%s := #%d\n", l_name, r_value);
		} else if (tmp->code->u.assign.right->kind == ADDRESS) {
			fprintf(fp, "%s := &%s\n", l_name, r_name);
		} else if (tmp->code->u.assign.right->kind == DER) {
			fprintf(fp, "%s := *%s\n", l_name, r_name);
		}
	} else if (tmp->code->u.assign.left->kind == DER) {
		if (tmp->code->u.assign.right->kind == VARIABLE) {
			fprintf(fp, "*%s := %s\n", l_name, r_name);
		} else if (tmp->code->u.assign.right->kind == CONSTANT) {
			fprintf(fp, "*%s := #%d\n", l_name, r_value);
...
void gen_ADD1(InterCodes tmp, FILE* fp){
